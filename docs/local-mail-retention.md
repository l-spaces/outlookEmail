...
While cleanup is running, duplicate clear requests should reuse the current running status instead of starting another destructive job. Once cleanup succeeds, the retained normal mail table should be empty and disabled local-list requests should remain blocked with the disabled local-storage message.
